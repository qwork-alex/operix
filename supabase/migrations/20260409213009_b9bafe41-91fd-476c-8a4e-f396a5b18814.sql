...
BEGIN
  ...
  -- Extract raw token string for logging
  ...
  VALUES ('auth', 'SIGNUP_TRIGGER_DEBUG', NEW.id,
    jsonb_build_object(
      'email', NEW.email,
      'raw_invite_token', COALESCE(v_raw_token, 'NULL'),
      'has_metadata', (NEW.raw_user_meta_data IS NOT NULL)::text,
      'metadata_keys', COALESCE((SELECT jsonb_agg(key) FROM jsonb_each(COALESCE(NEW.raw_user_meta_data, '{}'::jsonb))), '[]'::jsonb)
    ));

  -- Check if signup includes an invite token in metadata
  BEGIN
    v_invite_token := NULLIF(v_raw_token, '')::UUID;
  EXCEPTION WHEN others THEN
    v_invite_token := NULL;
    INSERT INTO public.backend_event_logs (table_name, action, row_id, payload)
    VALUES ('auth', 'SIGNUP_TOKEN_PARSE_ERROR', NEW.id,
      jsonb_build_object('raw_value', COALESCE(v_raw_token, 'NULL')));
  END;
  ...
  IF v_invite_token IS NOT NULL THEN
    ...
    INSERT INTO public.backend_event_logs (table_name, action, row_id, payload)
    VALUES ('auth', 'SIGNUP_INVITE_CHECK', NEW.id,
      jsonb_build_object('invite_token', v_invite_token::text, 'invite_exists', v_invite_exists::text));
  END IF;
