...
BEGIN
  SELECT COALESCE(display_name, email, 'Sistema')
    INTO v_actor
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    IF NEW.status IS DISTINCT FROM OLD.status THEN
      INSERT INTO public.production_events(
        production_order_id, workspace_id, actor_user_id, actor_name, event_type, from_value, to_value
      )
      VALUES (
        NEW.id,
        NEW.workspace_id,
    ...
    IF NEW.client_name IS DISTINCT FROM OLD.client_name THEN
      v_changes := v_changes || jsonb_build_object('client_name', jsonb_build_object('from', OLD.client_name, 'to', NEW.client_name));
    END IF;
    IF NEW.platform IS DISTINCT FROM OLD.platform THEN
      v_changes := v_changes || jsonb_build_object('platform', jsonb_build_object('from', OLD.platform, 'to', NEW.platform));
    END IF;
    IF NEW.insurer IS DISTINCT FROM OLD.insurer THEN
      v_changes := v_changes || jsonb_build_object('insurer', jsonb_build_object('from', OLD.insurer, 'to', NEW.insurer));
    END IF;
    IF NEW.license_plate IS DISTINCT FROM OLD.license_plate THEN
      v_changes := v_changes || jsonb_build_object('license_plate', jsonb_build_object('from', OLD.license_plate, 'to', NEW.license_plate));
    END IF;
    IF NEW.vin IS DISTINCT FROM OLD.vin THEN
      v_changes := v_changes || jsonb_build_object('vin', jsonb_build_object('from', OLD.vin, 'to', NEW.vin));
    END IF;
    ...
    IF NEW.color IS DISTINCT FROM OLD.color THEN
      v_changes := v_changes || jsonb_build_object('color', jsonb_build_object('from', OLD.color, 'to', NEW.color));
    END IF;
    IF NEW.notes IS DISTINCT FROM OLD.notes THEN
      v_changes := v_changes || jsonb_build_object('notes', true);
    END IF;
    IF NEW.due_at IS DISTINCT FROM OLD.due_at THEN
      v_changes := v_changes || jsonb_build_object('due_at', jsonb_build_object('from', OLD.due_at, 'to', NEW.due_at));
    END IF;

    IF v_changes <> '{}'::jsonb THEN
      INSERT INTO public.production_events(
        production_order_id, workspace_id, actor_user_id, actor_name, event_type, payload
      )
      VALUES (
        NEW.id,
        NEW.workspace_id,
...
DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime DROP TABLE public.production_events;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;

DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime DROP TABLE public.production_photos;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;