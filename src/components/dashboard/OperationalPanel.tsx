...
/* ---------- Phase 6: demand forecast — isolated, lazy, idle-deferred ----------
 * The actual UI lives in ./DemandBlock (lazy chunk). This wrapper guarantees:
 *   - render only after browser idle (post-paint)
 *   - errors / chunk-load failures are contained (ErrorBoundary + Suspense)
 *   - the rest of the panel renders even if demand intelligence is unavailable
 */
function DeferredDemandSection({
  metadata, color,
}: { metadata: Record<string, any> | null | undefined; color: string }) {
  const idle = useIdleMount(400);
  if (!idle) return null;
  return (
    <ErrorBoundary fallback={null}>
      <Suspense fallback={null}>
        <DemandBlock metadata={metadata} color={color} />
      </Suspense>
    </ErrorBoundary>
  );
}
