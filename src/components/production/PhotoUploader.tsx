...
interface Props { orderId: string; fixedCategory?: PhotoCategory; hideOthers?: boolean; readOnly?: boolean; }

export function PhotoUploader({ orderId, fixedCategory, hideOthers, readOnly }: Props) {
      ...
      {!readOnly && (
        <div className="flex items-center gap-2">
          ...
          <label className="flex-1">
            ...
            <Button asChild variant="outline" className="w-full cursor-pointer" disabled={busy}>
              <span><Upload className="h-4 w-4 mr-2" /> {busy ? "Enviando…" : "Galeria"}</span>
            </Button>
          </label>
          <label>
            <input type="file" accept="image/*" capture="environment" className="hidden" disabled={busy}
              onChange={(e) => e.target.files && handleFiles(e.target.files)} />
            <Button asChild variant="outline" className="cursor-pointer" disabled={busy}>
