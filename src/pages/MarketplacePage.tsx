...
export default function MarketplacePage() {
  ...
  const cats = useMemo(
    () => [["all", "Todos"] as const, ...Object.entries(CATEGORY_META).map(([k, m]) => [k, m.label] as const)],
    [],
  );
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Tabs value={scope} onValueChange={(v) => setScope(v as any)}>
          <TabsList>
            <TabsTrigger value="all">Descobrir</TabsTrigger>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-border/60 bg-muted/10 p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {scope === "mine" ? "Publique seu primeiro anúncio." : "Tente alterar os filtros ou voltar mais tarde."}
          </p>
          {scope === "mine" && (
