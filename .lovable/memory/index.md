...
## Memories
- [Strict role separation](mem://auth/strict-role-separation) — DB triggers enforce workspace owner = admin, admin membership requires admin role, deletion blocked while owning a workspace; signup defaults: self→admin, invited/admin-provisioned→no default
- [Platforms Operational](mem://features/platforms-operational) — canonical platforms table, lifecycle states (active/paused/archived/degraded), state_log audit, optional FK on service_orders, realtime PlatformsPanel + OperationalKPIs + OperationalEventsStream on /
...
- [Phase 6D Invoices](mem://features/billing/phase-6d-invoices) — Additive engine on platform_invoices: PDF via jsPDF edge fn, TVA engine (with/no/reverse_charge), bank snapshot (Wise vs empresa), invoice_events audit, invoice_email_queue dispatcher, CSV/JSON exports.
- [Phase 6E Automation](mem://features/billing/phase-6e-automation) — Daily 03:00 UTC cron runs run_subscription_automation; dunning_events trigger auto-enqueues stage-specific emails (reminder/warning/limited/suspension); 5-min email dispatcher cron; AutomationPanel shows last run + cron status.
- [Phase 6F Manual Payments](mem://features/billing/phase-6f-manual-payments) — Wise/SEPA/bank fallback: manual_bank_transfers + payment-proofs bucket, dynamic bank routing by vat_mode (no_vat→Wise personal, else business), owner approve/reject RPCs that mark invoice paid; ManualPaymentDialog + ManualPaymentsReview UI.
- [Automation Engine](mem://features/automation-engine) — Workspace-scoped operational automations: rules → async queue → executions → dead-letter. Edge fn drains every minute. Anti-recursion via depth+correlation. SAFE MODE: no auth/RBAC changes.
